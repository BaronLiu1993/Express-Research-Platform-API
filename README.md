...
The detailed system architecture and design notes are included below (also available as `API/ARCHITECTURE.md`).
...
graph LR
	...
	Browser -->|HTTPS REST / JWT| API[Express API\n(API/index.js)]

	subgraph APIServer[Application Server]
		API --> Routers[Routers (/auth,/email,/snippets,...)]
		Routers --> Services[Services\n(auth, google, email, storage)]
		API --> QueueClients[BullMQ Queues (producer)]
	end

	Services --> Supabase[(Supabase / Postgres)]
	Services --> Gmail[Google Gmail API]
	Services --> Drive[Google Drive API]
	Services --> OpenAI[OpenAI / Embeddings]

	QueueClients -->|job enqueues| Redis[(Redis)]
	Redis --> Workers[Workers (sendWorker, draftWorker, ...)]
	Workers --> Services
	Workers --> Gmail
	Workers --> Supabase
	Workers --> Storage[Storage / Presigned URLs]

	classDef infra fill:#f3f4f6,stroke:#333,stroke-width:1px;
	class Redis,Workers,Supabase,Gmail,Drive,OpenAI,Storage infra;

	%% Notes: arrows indicate primary interactions/data flow
```
